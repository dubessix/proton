...
import {
  ...
  readEmails,
  sendEmail,
} from "@renderer/functions/gmail-manager-api";
...
const CATEGORIZED_TOOLS = {
  ...
  WEB_INTELLIGENCE: [
    ...
    },
    {
      ...
      parameters: {
        properties: { port: { type: "NUMBER", description: "e.g. 3000" } },
      },
    },
    {
      name: "close_wormhole",
  ...
  COMMUNICATION: [
    ...
    },
    {
      ...
      parameters: {
        properties: {
          max_results: { type: "NUMBER", description: "Default is 5" },
        },
      },
    },
    {
  ...
  MOBILE_LINK: [
    ...
    },
    {
      ...
      parameters: {
        properties: { setting: { type: "STRING" }, state: { type: "BOOLEAN" } },
      },
    },
    {
      name: "send_whatsapp",
...
function Editor() {
  ...
  const openParameterEditor = useCallback(
  ...
  return (
    <div className="flex h-full w-full bg-[#09090b] relative overflow-hidden">
              ...
              <div key={category} className="mb-6">
                ...
                <div className="flex flex-col gap-2">
                  {tools.map((tool: any) => (
                    <div
                      key={tool.name}
                      className={`flex items-center gap-3 p-2 rounded-lg cursor-grab transition-all group ${
                        isDark
                          ? 'bg-[#18181b] border border-[#27272a] hover:border-emerald-500/50 hover:bg-[#27272a]/50'
                          : 'bg-white/40 border border-black/10 hover:border-emerald-500/30 hover:bg-white/50'
                      }`}
                      draggable
                      onDragStart={(e) =>
                    ...
                    >
                      <div className={`p-1.5 rounded shadow-inner border ${
                        isDark 
                      ...
                      <div className="flex flex-col">
                        <span className={`text-[10px] font-bold tracking-widest uppercase group-hover:transition-colors ${
                          isDark
                            ? 'text-zinc-300 group-hover:text-white'
                            : 'text-[#5A6B5A] group-hover:text-[#1A1F1A]'
                        }`}>
      ...
      <button
        ...
        className={`absolute top-1/2 left-0 transform -translate-y-1/2 border border-l-0 p-2 rounded-r-lg z-50 transition-colors ${
          isDark
            ? 'bg-[#111113] border-[#27272a] text-zinc-600 hover:text-emerald-500'
            : 'bg-white/50 border-black/10 text-[#8FA08F] hover:text-emerald-500'
        }`}
      >
        {isSidebarOpen ? (
          <RiLayoutColumnLine size={18} />
        ) : (
          <RiLayoutColumnFill size={18} />
        )}
      </button>
        ...
        <div className="absolute top-4 left-4 z-10 flex items-center gap-3 shadow-2xl">
          <button
            ...
            className={`p-3 rounded-lg border transition-colors cursor-pointer ${
              isDark
                ? 'bg-[#18181b] border-[#27272a] text-zinc-600 hover:text-emerald-500 hover:border-emerald-500/50'
                : 'bg-white/40 border-black/10 text-[#8FA08F] hover:text-emerald-500 hover:border-emerald-500/30'
            }`}
          ...
          <input
            ...
            className={`px-4 py-2 rounded-lg text-sm outline-none font-bold tracking-wide w-64 shadow-inner border transition-colors ${
              isDark
                ? 'bg-[#18181b] border-[#27272a] text-white focus:border-emerald-500'
                : 'bg-white/50 border-black/10 text-[#1A1F1A] focus:border-emerald-500'
            }`}
          />

          <button
            onClick={runMacroManually}
            className={`px-5 py-2 rounded-lg text-[11px] font-black tracking-widest transition-all flex items-center gap-2 cursor-pointer shadow-lg border ${
              isDark
                ? 'bg-[#18181b] hover:bg-[#27272a] text-emerald-400 border-[#27272a] hover:border-emerald-500/50'
                : 'bg-white/40 hover:bg-white/60 text-emerald-600 border-black/10 hover:border-emerald-500/30'
            }`}
          >
