import { r as __exportAll } from "./rolldown-runtime-Cf1989ZW.js";
	...
	async getResponse(response, url, headers, outputType, signal) {
		...
		for (let step = 0; step < 5; step++) {
			...
			if (typeof payload === "object" && payload && "status" in payload && typeof payload.status === "string" && payload.status === "Ready" && "result" in payload && typeof payload.result === "object" && payload.result && "sample" in payload.result && typeof payload.result.sample === "string") {
				...
				return await (await fetch(payload.result.sample, { signal })).blob();
			}
		}
...
var TOGETHER_VIDEO_PENDING_STATUSES = new Set(["queued", "in_progress"]);
...
var multimolecule = (model) => {
	const exampleText = (model.widgetData?.[0])?.text;
